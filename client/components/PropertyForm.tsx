...
import { Plus, X, Upload, Loader2, MapPin, RotateCcw, RefreshCw } from 'lucide-react'
...
export function PropertyForm({ property, onSuccess, onCancel }: PropertyFormProps) {
    ...
    async function clientAction(formData: FormData) {
        ...
        if (!isInsidePhilippines) {
            alert("The selected location is outside the Philippines. Please select a valid location.")
            return
        }

        if (!address.includes("Philippines")) {
            alert("The address does not appear to be in the Philippines. Please check the location.")
            return
        }
        ...
        <>
            <form action={clientAction} className="grid gap-6 p-6">
                ...
                <div className="mt-2 pt-4 border-t border-gray-100 flex justify-end">
                    <Button type="button" variant="ghost" onClick={onCancel} className="mr-2 hover:bg-gray-100/80 rounded-xl font-medium text-gray-600">Cancel</Button>
                    <Button
                        type="submit"
                        disabled={uploading}
